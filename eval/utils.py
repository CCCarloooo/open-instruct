...
def load_vicuna_lm_and_tokenizer(
        model_name_or_path, 
    ...
    from transformers import LlamaForCausalLM, AutoTokenizer, OPTForCausalLM, GPTNeoXForCausalLM
    ...
    if gptq_model:
        ...
        if device_map:
            model = LlamaForCausalLM.from_pretrained(model_name_or_path, device_map=device_map, torch_dtype=torch_dtype)
        else:
            model = LlamaForCausalLM.from_pretrained(model_name_or_path, torch_dtype=torch_dtype)
